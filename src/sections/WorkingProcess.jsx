...
import WorkingProcessAccordion from "../components/WorkingProcessAccordion";
...
const WorkingProcess = () => {
  ...
  const content = [
    ...
    {
      index: 2,
      title: "Research and Startegy Development",
      description:
    ...
    <section className="p-5 flex justify-center overflow-hidden mt-11 ">
      <div className="bg-white max-w-7xl mx-auto w-full">
        <div className="flex flex-col items-center gap-10 sm:flex-row mb-20">
          <h1 className="bg-lime-300 text-center sm:text-left text-3xl sm:text-4xl">
            Our Working Process
          </h1>
          <p className="w-auto text-center sm:text-left sm:w-125">
        ...
        <div className="flex flex-col gap-5">
          {content.map((item) => (
            <WorkingProcessAccordion key={item.index} item={item} openIndex={openIndex} handleToggle={handleToggle}/>
          ))}
        </div>
      </div>
    </section>
